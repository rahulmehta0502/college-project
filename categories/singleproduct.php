...
if(isset($_GET['action']))
	...
	function cartinsert() {
		...
		while($row=mysqli_fetch_array($result1)) {
			if($row['userid']==$userid && $row['pid']==$productid)
			$flag=1;
		}
		if($flag!=1){
		$sql="insert into cart(cartid,userid,pid)values('$cartid','$userid','$productid')";
		if(mysqli_query($conn,$sql)) 
			echo '<script>alert("cart inserted")</script>';
		else
...
if(array_key_exists('bid',$_POST)) {
	...
	$bid=$_POST['bidprice'];
	$flag=1;
	$sql1="select * from buyer where b_id='$buyer'";
	// && p_id='$pid' && remarks!='1' && bid_price='$bid'
	$result1=mysqli_query($conn,$sql1) or die( mysqli_error($conn));
	while($row1=mysqli_fetch_array($result1)) {
	 if($row1['remarks']==0 || $row1['remarks']==1 ){
		$flag=0;
	 }

	}
	if($flag==1){
	$sql="insert into buyer (b_id,p_id,date,bid_price) values('$buyer','$pid','$date','$bid')";
	if(mysqli_query($conn,$sql)){
		echo '<script>alert("Bid Successfully inserted")</script>';
	}
	}
}
?>
			...
			<div class="agile-its-header">
				...
				<div class="agileits_search">
					...
					<select onchange="location=this.options[this.selectedIndex].value;" id="agileinfo_search" name="agileinfo_search" required="">
							...
							<option value="http://localhost:8080/ip/zhatpat/categories/mobiles.php">Mobiles</option>
							<option value="http://localhost:8080/ip/zhatpat/categories/electronics.php">Electronics & Appliances</option>
							<option value="http://localhost:8080/ip/zhatpat/categories/Cars.php">Cars</option>
							<option value="http://localhost:8080/ip/zhatpat/categories/Bikes.php">Bikes</option>
							<option value="http://localhost:8080/ip/zhatpat/categories/Furnitures.php">Furnitures</option>
							<option value="http://localhost:8080/ip/zhatpat/categories/Books.php">Books</option>
				...
				<a class="post-w3layouts-ad" href=" http://localhost:8080/ip/zhatpat/useraccount/post-ad.php">Post Free Ad</a>
				</div>	
			...
			<div class="product-desc">
				...
				<div class='col-md-5 product-details-grid'>
					<div class='item-price'>";
						...
						<div class='itemtype'>
						<h4>
						<a href='http://localhost:8080/ip/zhatpat/categories/singleproduct.php?action=cartinsert& userid=".$userid." && pid=".$pid." && sellid=".$sellid." && id=".$picture."'> 
						<button class='post-w3layouts-ad' name='cart'> Cart </button></form></a>
						<button class='pd-setting' name=payment value=><a href='http://localhost:8080/ip/zhatpat/useraccount/totalpayment.php?pid=".$pid."&bidprice=".$row1['price']."'> Payment  </button></a>
